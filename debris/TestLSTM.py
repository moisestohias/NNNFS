...
def _lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    i = sigmoid(a[:, 0:H])
    f = sigmoid(a[:, H:2*H])
    o = sigmoid(a[:, 2*H:3*H])              # He is mistakingly swapping o & g
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b): # swapping o/g
    _, H = prev_h.shape
    a = prev_h.dot(Wh) + x.dot(Wx) + b      # (1, 4*hidden_dim)
    i = sigmoid(a[:, 0:H])
    f = sigmoid(a[:, H:2*H])
    g = np.tanh(a[:, 2*H:3*H])              # He is mistakingly swapping o & g
    o = sigmoid(a[:, 3*H:4*H])              # (1, hidden_dim)
    next_c = f * prev_c + i * g             # (1, hidden_dim)
    next_h = o * (np.tanh(next_c))          # (1, hidden_dim)
    cache = x, prev_h, prev_c, Wx, Wh, b, a, i, f, o, g, next_c
    return next_h, next_c, cache

def __lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
...
def _lstm_step_backward(dnext_h, dnext_c, cache):
    """
